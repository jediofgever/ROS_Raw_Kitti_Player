#include <kitti_ros/segmenters_lib/region_growing_segmenter.hpp>
...
RegionGrowingSegmenter::RegionGrowingSegmenter(const SegmenterParams& params)
    : kd_tree_(new pcl::search::KdTree<Point>), params_(params) {
...
void RegionGrowingSegmenter::segment(const PointCloudPtr pc,
    ...
    *cloud = *pc;
    ...
    for (size_t i = 0u; i < normals->size(); ++i) {
        ...
        params_.rg_curvature_threshold) { ground_indices->indices.push_back(i);
        }*/
    }

#include <kitti_ros/segmenters_lib/region_euclidean_segmenter.hpp>
...
void RegionEuclideanSegmenter::segment(
    ...
    *cloud = cloud_in;
    ...
    for (size_t region = 0u; region < params_.rec_region_size; ++region) {
        ...
        if (nested_regions[region].size() > params_.rec_min_cluster_size) {
            ...
            for (std::vector<pcl::PointIndices>::const_iterator citer =
                ...
                citer->indices.begin(); float distMin =
                common::calcCylinderDist(cloud->points[*piter]); float distMax =
                common::calcCylinderDist(cloud->points[*piter]); for ( ; piter
                != citer->indices.end(); ++piter) {
                    cluster->points.push_back(cloud->points[*piter]);
                    float dist =
                common::calcCylinderDist(cloud->points[*piter]); if (dist <
    ...
    if (params_.rec_use_region_merge) {
                ...
                for (size_t in_idx = 0;
                    ...
                    if (common::bbox::groundBoxIoU(gbox_in, gbox_out) >
                        ...
                        *(*cloud_clusters)[clusterId_outside] +=
                        ...
                        // clusterId_inside << " to #" << clusterId_outside <<
                        // ".");
        ...
        if (had_merged) {
            std::vector<PointICloudPtr>::iterator iter =
                (*cloud_clusters).begin();

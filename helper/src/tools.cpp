...
Tools::Tools() {
    ...
    TRANS_VELO_TO_CAM << 7.533745000000e-03, -9.999714000000e-01,
        ...
        -2.717806000000e-01, 0, 0, 0, 0;

    TRANS_CAM_TO_RECTCAM = MatrixXf::Zero(4, 4);
    TRANS_CAM_TO_RECTCAM << 9.999239000000e-01, 9.837760000000e-03,
        -7.445048000000e-03, 0, -9.869795000000e-03, 9.999421000000e-01,
        -4.278459000000e-03, 0, 7.402527000000e-03, 4.351614000000e-03,
        9.999631000000e-01, 0, 0, 0, 0, 1;

    TRANS_RECTCAM_TO_IMAGE = MatrixXf::Zero(3, 4);
    TRANS_RECTCAM_TO_IMAGE << 7.215377000000e+02, 0.000000000000e+00,
        6.095593000000e+02, 4.485728000000e+01, 0.000000000000e+00,
        7.215377000000e+02, 1.728540000000e+02, 2.163791000000e-01,
    ...
    SEMANTIC_NAMES = std::vector<std::string>{
        ...
        // Dynamic objects
        "Pedestrian", "Rider", "Car", "Truck", "Bus", "Train", "Motocycle",
        "Bicycle"};

    SEMANTIC_COLOR_TO_CLASS = std::map<int, int>{// Static objects
                                                 {320, 0},
                                                 {511, 1},
                                                 {210, 2},
                                                 {260, 3},
    ...
    SEMANTIC_KERNEL_SIZE << 1,  // Pedestrian
        2,                      // Rider
        1,                      // Car
        4,                      // Truck
        5,                      // Bus
...
MatrixXf Tools::getImage2DBoundingBox(const Point &point, const float width,
                                      const float height) {
...
MatrixXf Tools::getImage2DBoundingBox(const Object o) {
    ...
    if (max_x > 1237) max_x = 1237.0;
    if (min_y < 0) min_y = 0.0;
    if (max_y > 370) max_y = 370.0;
...
MatrixXf Tools::transformVeloToImage(const MatrixXf &velo_points) {
    return transformRectCamToImage(TRANS_CAM_TO_RECTCAM * TRANS_VELO_TO_CAM *
                                   velo_points);
}

void Tools::EulerAngleToQuaternion(double ang, double &x, double &y, double &z,
                                   double &w) {
